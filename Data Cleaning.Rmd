...
# Animation for % registered over time 
...
time_elapse <- all_years_registration %>%
  full_join(acs_2015_g_clean, by = "county") 
...
```{r}
# Growth in each county vs. demographics in 2015
play5 <- play4 %>%
  filter(!diff == "0")

  growth_regression <-
            lm(diff ~ Perc_white * Median_income * Median_age, data = play5)
